"""Generate HTML report for malicious PDF detection."""
...
def plot_cm(cm, labels, title):
    ...
    ax.imshow(cm, interpolation="nearest", cmap=plt.cm.Purples)
...
def plot_bar(df, metric, title, color="#7c3aed"):
...
def generate_report():
    if not METRICS_JSON.exists(): print("Run train_pdf_model.py first."); return
    ...
    cm_imgs = {n: plot_cm(np.array(d.get("confusion_matrix",[[0,0],[0,0]])), ["Benign","Malicious"], n)
               for n,d in metrics.items()}
...
<title>Malicious PDF Detection Report</title>
...
<p class="subtitle">31 PDF Structural Features — {len(metrics)} Models</p>
