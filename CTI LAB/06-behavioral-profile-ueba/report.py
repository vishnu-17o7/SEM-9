"""Generate HTML report for behavioral profile anomaly detection."""
...
def plot_bar(results_df, metric, title, color="#2563eb"):
...
def generate_report():
    if not METRICS_JSON.exists():
        print("Run build_profiles.py first."); return
    with open(METRICS_JSON) as f: metrics = json.load(f)
    rows = [{"Detector": n, "Accuracy": d["accuracy"], "Precision": d["precision"],
             "Recall": d["recall"], "F1": d["f1"], "ROC-AUC": d["roc_auc"],
             "Anomalies": d["anomaly_count"]} for n, d in metrics.items()]
    df = pd.DataFrame(rows)

    cm_imgs = {n: plot_cm(np.array(d.get("confusion_matrix", [[0,0],[0,0]])), ["Normal", "Anomaly"], n)
               for n, d in metrics.items()}
    f1_img = plot_bar(df, "F1", "F1 Score by Detector")
    prec_img = plot_bar(df, "Precision", "Precision by Detector", "#059669")
    recall_img = plot_bar(df, "Recall", "Recall by Detector", "#dc2626")

    trows = "".join(f'<tr><td><strong>{r["Detector"]}</strong></td><td>{r["Accuracy"]:.4f}</td>'
                    f'<td>{r["Precision"]:.4f}</td><td>{r["Recall"]:.4f}</td>'
                    f'<td>{r["F1"]:.4f}</td><td>{r["ROC-AUC"]:.4f}</td>'
                    f'<td>{int(r["Anomalies"])}</td></tr>'
...
<title>UEBA Anomaly Detection Report</title>
...
<div class="container">
<h1>UEBA — Behavioral Anomaly Detection</h1>
<p class="subtitle">User and Entity Behavior Analytics on Enterprise Activity Logs</p>
...
<div class="summary-card"><div class="label">Total Anomalies</div><div class="value">{int(df["Anomalies"].sum())}</div></div>
</div>
<h2 class="section-title">Detector Performance</h2>
<table><thead><tr><th>Detector</th><th>Accuracy</th><th>Precision</th><th>Recall</th><th>F1</th><th>ROC-AUC</th><th>Anomalies</th></tr></thead>
