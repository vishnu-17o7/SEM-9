"""Generate HTML report for network intrusion detection."""
...
def generate_report():
    if not METRICS_JSON.exists(): print("Run train_nids.py first."); return
    ...
    cm_imgs = {n: plot_cm(np.array(d.get("confusion_matrix",[[0,0],[0,0]])),["Normal","Attack"],n) for n,d in metrics.items()}
    f1_img = plot_bar(df,"F1","F1 Score by Model"); roc_img = plot_bar(df,"ROC-AUC","ROC-AUC by Model","#059669"); acc_img = plot_bar(df,"Accuracy","Accuracy by Model","#d97706")
    ...
    html = f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Network Intrusion Detection</title><style>*{{margin:0;padding:0;box-sizing:border-box}}
...
<p class="subtitle">NSL-KDD Dataset — {len(metrics)} Models</p>
...
<div class="charts"><div class="chart-card"><h2>F1 Score</h2><img src="data:image/png;base64,{f1_img}"></div><div class="chart-card"><h2>ROC-AUC</h2><img src="data:image/png;base64,{roc_img}"></div><div class="chart-card"><h2>Accuracy</h2><img src="data:image/png;base64,{acc_img}"></div></div>
