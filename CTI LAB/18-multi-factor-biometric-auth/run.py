"""Unified entry point for Multi-Factor Biometric Auth.

Usage:
    python run.py <command> [options]

Commands:
    data      Set up MFA (enroll users, generate TOTP + face embeddings)
    classify  Run authentication (CLI)
    web       Launch Flask web UI (port 5003)
...
CLASSIFY_SCRIPT: str | None = "mfa_auth.py"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Multi-Factor Biometric Auth -- unified runner")
    parser.add_argument("command", nargs="?", default="steps",
                        help="data | classify | web | all | steps")
