"""Generate HTML report for botnet classification."""
...
def plot_cm(cm, labels, title):
    ...
    ax.imshow(cm, interpolation="nearest", cmap=plt.cm.RdPu)
...
def generate_report():
    if not METRICS_JSON.exists(): print("Run train_botnet_model.py first."); return
    ...
    cm_imgs = {n: plot_cm(np.array(d.get("confusion_matrix",[[0,0],[0,0]])),["Normal","Botnet"],n) for n,d in metrics.items()}
    ...
    html = f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Botnet Traffic Classification</title><style>*{{margin:0;padding:0;box-sizing:border-box}}
...
<div class="container">
<h1>Botnet Traffic Classification</h1>
<p class="subtitle">32 Flow-based Traffic Features — {len(metrics)} Models</p>
