"""
Train ransomware behavior detection model.
"""
...
def load_data():
    csv_path = DATA_DIR / "ransomware_activity.csv"
    if not csv_path.exists():
        raise RuntimeError("Run simulate_ransomware.py first.")
    df = pd.read_csv(csv_path)
    from simulate_ransomware import FEATURE_NAMES
    ...
    return X[train_idx], X[test_idx], y[train_idx], y[test_idx], feature_cols, X, y, groups, train_idx, test_idx
...
def evaluate(name, pipe, Xtr, Xte, ytr, yte):
    ...
    return ModelResult(name=name, family=pipe.named_steps.get("clf",pipe).__class__.__name__,
                       ...
                       predict_time_s=round(pred_t,3), confusion_matrix=cm)

def main():
    RESULTS_DIR.mkdir(exist_ok=True); PREDICTIONS_DIR.mkdir(exist_ok=True); MODEL_DIR.mkdir(exist_ok=True)
    print("=" * 60); print("  Ransomware Behavior Detection — Model Training"); print("=" * 60)
    Xtr, Xte, ytr, yte, fcols, Xall, yall, groups, train_idx, test_idx = load_data()
    ...
    for r in top3:
        try:
            p = load(MODEL_DIR / f"{r.name}.joblib"); est.append((r.name, p))
        except: pass
    if len(est) >= 2:
        t0 = time.perf_counter()
        ens = VotingClassifier(estimators=est, voting="soft"); ens.fit(Xtr, ytr)
        train_t = time.perf_counter() - t0
        t0 = time.perf_counter(); yp = ens.predict(Xte); pred_t = time.perf_counter() - t0
        ypr = ens.predict_proba(Xte)[:, 1]
        er = ModelResult(name="VotingEnsemble", family="ensemble",
            ...
            confusion_matrix=confusion_matrix(yte,yp).tolist(), notes=f"Soft-vote of {', '.join(r.name for r in top3)}")
        results.append(er)
        dump(ens, MODEL_DIR / "VotingEnsemble.joblib")
    ...
    write_evaluation_manifest(
        RESULTS_DIR,
        project="09-ransomware-behavior-detector",
        dataset={**source_context(DATA_DIR, "Scenario-based synthetic Windows ransomware telemetry"), "rows": len(Xall), "limitations": "Synthetic fallback; use real Windows telemetry before making operational claims."},
        split={"strategy": "duplicate-free StratifiedGroupKFold holdout by scenario_id", "seed": RANDOM_STATE, "group_key": "scenario_id", "partitions": {"train": len(train_idx), "test": len(test_idx)}, "train_groups": int(np.unique(groups[train_idx]).size), "test_groups": int(np.unique(groups[test_idx]).size)},
