"""Unified entry point for Email Phishing NLP.

Usage:
    python run.py <command> [options]

Commands:
    data      Download phishing email dataset
    train     Train NLP phishing model
    report    Generate HTML report
    classify  Classify an email (interactive CLI)
    web       Launch web UI (port 5005)
...
COMMANDS: dict[str, list[list[str]]] = {
    ...
    "train": [["train_phishing_model.py"]],
    "report": [["report.py"]],
}

CLASSIFY_SCRIPT: str | None = "classify.py"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Email Phishing NLP -- unified runner")
    ...
    if cmd == "all":
        steps: list[list[str]] = [["download_data.py"], ["train_phishing_model.py"]]
        run_scripts(steps)
