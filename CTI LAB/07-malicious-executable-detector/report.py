"""Generate HTML report for PE malware detection."""
...
def plot_cm(cm, labels, title):
    ...
    ax.set_title(title, fontsize=11, pad=8); ax.set_xticks(range(len(labels)))
    ax.set_yticks(range(len(labels))); ax.set_xticklabels(labels); ax.set_yticklabels(labels)
    thresh = cm.max() / 2
...
def plot_bar(df, metric, title, color="#dc2626"):
    df_s = df.sort_values(metric, ascending=True)
    fig, ax = plt.subplots(figsize=(5, 3.5))
    ax.barh(df_s["Model"], df_s[metric], color=color, height=0.5)
...
def generate_report():
    if not METRICS_JSON.exists(): print("Run train_malware_model.py first."); return
    with open(METRICS_JSON) as f: metrics = json.load(f)
    rows = [{"Model": n, "Accuracy": d["accuracy"], "Precision": d["precision"],
             "Recall": d["recall"], "F1": d["f1"], "ROC-AUC": d["roc_auc"]}
            for n, d in metrics.items()]
    df = pd.DataFrame(rows)

    cm_imgs = {n: plot_cm(np.array(d.get("confusion_matrix", [[0,0],[0,0]])), ["Benign", "Malware"], n)
               for n, d in metrics.items()}
    f1_img = plot_bar(df, "F1", "F1 Score by Model", "#2563eb")
    roc_img = plot_bar(df, "ROC-AUC", "ROC-AUC by Model", "#059669")
    acc_img = plot_bar(df, "Accuracy", "Accuracy by Model", "#d97706")

    trows = "".join(f'<tr><td><strong>{r["Model"]}</strong></td><td>{r["Accuracy"]:.4f}</td>'
                    f'<td>{r["Precision"]:.4f}</td><td>{r["Recall"]:.4f}</td>'
                    f'<td>{r["F1"]:.4f}</td><td>{r["ROC-AUC"]:.4f}</td></tr>'
                    for _, r in df.sort_values("F1", ascending=False).iterrows())
    cm_sec = "".join(f'<div class="cm-card"><h3>{n}</h3><img src="data:image/png;base64,{v}"></div>'
                     for n, v in sorted(cm_imgs.items()))

    html = f"""<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>PE Malware Detection Report</title>
...
.chart-card h2{{font-size:.9rem;margin-bottom:.5rem;color:#334155}}
...
.cm-card h3{{font-size:.85rem;color:#475569;margin-bottom:.5rem}}
.cm-card img{{max-width:100%}}
.section-title{{font-size:1.1rem;font-weight:600;margin-bottom:1rem;margin-top:1.5rem;color:#0f172a}}</style></head><body>
<div class="container">
<h1>PE Malware Detection</h1>
<p class="subtitle">Binary Classification on Synthetic PE Features ({len(metrics)} models)</p>
<div class="summary-cards">
<div class="summary-card"><div class="label">Best F1</div><div class="value">{df["F1"].max():.4f}</div></div>
<div class="summary-card"><div class="label">Best ROC-AUC</div><div class="value">{df["ROC-AUC"].max():.4f}</div></div>
<div class="summary-card"><div class="label">Best Accuracy</div><div class="value">{df["Accuracy"].max():.4f}</div></div>
</div>
<h2 class="section-title">Model Performance</h2>
<table><thead><tr><th>Model</th><th>Accuracy</th><th>Precision</th><th>Recall</th><th>F1</th><th>ROC-AUC</th></tr></thead>
...
<div class="chart-card"><h2>ROC-AUC</h2><img src="data:image/png;base64,{roc_img}"></div>
<div class="chart-card"><h2>Accuracy</h2><img src="data:image/png;base64,{acc_img}"></div>
</div>
