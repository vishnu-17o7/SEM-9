"""Generate HTML report for crypto-ransomware detection."""
...
def plot_cm(cm, labels, title):
    ...
    ax.imshow(cm, interpolation="nearest", cmap=plt.cm.OrRd)
...
def generate_report():
    if not METRICS_JSON.exists(): print("Run train_crypto_model.py first."); return
    with open(METRICS_JSON) as f: metrics = json.load(f)
    rows = [{"Model":n,"Accuracy":d["accuracy"],"Precision":d["precision"],"Recall":d["recall"],"F1":d["f1"],"ROC-AUC":d["roc_auc"]} for n,d in metrics.items()]
    df = pd.DataFrame(rows)
    cm_imgs = {n: plot_cm(np.array(d.get("confusion_matrix",[[0,0],[0,0]])),["Normal","Ransomware"],n) for n,d in metrics.items()}
    f1_img = plot_bar(df,"F1","F1 Score"); roc_img = plot_bar(df,"ROC-AUC","ROC-AUC","#059669")
    trows = "".join(f'<tr><td><strong>{r["Model"]}</strong></td><td>{r["Accuracy"]:.4f}</td><td>{r["Precision"]:.4f}</td><td>{r["Recall"]:.4f}</td><td>{r["F1"]:.4f}</td><td>{r["ROC-AUC"]:.4f}</td></tr>' for _,r in df.sort_values("F1",ascending=False).iterrows())
    cm_sec = "".join(f'<div class="cm-card"><h3>{n}</h3><img src="data:image/png;base64,{v}"></div>' for n,v in sorted(cm_imgs.items()))
    html = f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Crypto-Ransomware Detection</title><style>*{{margin:0;padding:0;box-sizing:border-box}}
...
<div class="container">
<h1>Crypto-Ransomware Detection</h1>
<p class="subtitle">32 Encryption &amp; System Resource Features — {len(metrics)} Models</p>
...
<table><thead><tr><th>Model</th><th>Accuracy</th><th>Precision</th><th>Recall</th><th>F1</th><th>ROC-AUC</th></tr></thead><tbody>{trows}</tbody></table>
<div class="charts"><div class="chart-card"><h2>F1 Score</h2><img src="data:image/png;base64,{f1_img}"></div><div class="chart-card"><h2>ROC-AUC</h2><img src="data:image/png;base64,{roc_img}"></div></div>
